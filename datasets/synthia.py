...
def make_dataset(mode, maxSkip=0, cv_split=0):
    ...
    for mode in modes:
        ...
        if mode =='train':
            train_file = 'split_data/synthia_split_train.txt'
            with open(train_file, 'r') as f:
                lines = f.readlines()
            items = [line.strip() for line in lines]
        elif mode == 'val':
            val_file = 'split_data/synthia_split_val.txt'
            with open(val_file, 'r') as f:
                lines = f.readlines()
            items = [line.strip() for line in lines]
        else:
            raise NotImplementedError
    items = [(os.path.join(root, 'RGB', item), os.path.join(root, 'GT', 'LABELS', item)) for item in items]
