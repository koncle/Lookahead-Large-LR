...
def add_items(items, aug_items, cities, img_path, mask_path, mask_postfix, mode, maxSkip):
    ...
    for c in cities:
        c_items = [name.split(img_postfix)[0] for name in
                   os.listdir(os.path.join(img_path, c)) if '._' not in str(name)]
        for it in c_items:
...
def make_cv_splits(img_dir_name):
    ...
    trn_cities = ['train/' + c for c in os.listdir(trn_path) if os.path.isdir(os.path.join(trn_path, c))]
    val_cities = ['val/' + c for c in os.listdir(val_path) if os.path.isdir(os.path.join(val_path, c))]
...
def make_test_split(img_dir_name):
    ...
    test_cities = ['test/' + c for c in os.listdir(test_path) if os.path.isdir(os.path.join(test_path, c))]
...
def make_dataset(quality, mode, maxSkip=0, fine_coarse_mult=6, cv_split=0):
    ...
    if quality == 'coarse':
        ...
        for mode in modes:
            if mode == 'test':
                cv_splits = make_test_split(None)
