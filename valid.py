...
from utils.misc import update_bn
...
import numpy as np
import random
from datasets import cityscapes, synthia, mapillary, bdd100k
...
def colorize_mask(dataset, mask):
    if dataset == 'cityscapes':
        return cityscapes.colorize_mask(mask)
    elif dataset == 'synthia':
        return synthia.colorize_mask(mask)
    elif dataset == 'mapillary':
        return mapillary.colorize_mask(mask)
    elif dataset == 'bdd100k':
        return bdd100k.colorize_mask(mask)
    else:
...
def main():
    ...
    train_loader, _, _, extra_val_loaders, _ = datasets.setup_loaders(args)
    ...
    #     print('updateBN')
    # update_bn(train_loader, net, 400, device="cuda")  # TODO : reset to the resnet statistics?
    # if args.local_rank == 0:
...
def validate(val_loader, dataset, net, criterion, optim, scheduler, curr_epoch, writer, curr_iter, save_pth=True):
    ...
    for val_idx, data in enumerate(val_loader):
        ...
        # save the semgnetation prediction with the original image
        # num = len(val_loader)
        # if args.local_rank == 0 and val_idx % (num // 20) == 0:
        #     j = 0 # save one image
        #     path = Path('saved_images') / dataset
        #     path.mkdir(exist_ok=True, parents=True)
        #
        #     if True:
        #         pred = predictions[j]
        #         pred = pred.numpy()
        #         pred = colorize_mask(dataset, pred)
        #         # pred = Image.fromarray(pred)
        #         p = os.path.join('saved_images', dataset, 'regLA-' + img_names[j] + '.png')
        #         pred.save(p)
        #         print('Save image ', p)
