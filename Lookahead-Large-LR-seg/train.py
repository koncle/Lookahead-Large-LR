...
from utils.misc import AverageMeter, prep_experiment, evaluate_eval, fast_hist, update_bn, get_data
import datasets
import loss
import network
import optimizer
...
args.dist_url = args.dist_url + str(8100 + (int(time.time() % 1000)))
...
def main():
    ...
    while i < args.max_iter:
        ...
        if args.LA:
            i = lookahead_train(train_loader, net, optim, epoch, writer, scheduler, args.max_iter)
        else:
        ...
        if (args.dynamic and args.use_isw and epoch % (args.cov_stat_epoch + 1) == args.cov_stat_epoch) \
            ...
            for trial in range(args.trials):
                for dataset, val_loader in covstat_val_loaders.items():  # For get the statistics of covariance
                    validate_for_cov_stat(val_loader, dataset, net, criterion_val, optim, scheduler, epoch, writer, i, save_pth=False)
    ...
    if args.LA:
        if args.local_rank == 0:
            print('updateBN')
        update_bn(train_loader, net, 500, device="cuda")
        if args.local_rank == 0:
...
def train(train_loader, net, optim, curr_epoch, writer, scheduler, max_iter):
    ...
    for i, data in enumerate(train_loader):
        ...
        log(curr_epoch, curr_iter, f_cor_arr, i, optim, time_meter, train_loader, train_total_loss, writer, wt_loss)
...
def forward_once(aux_gt, batch_pixel_size, curr_epoch, gt, input, net, train_total_loss):
    if args.use_isw:
        outputs = net(input, gts=gt, aux_gts=aux_gt, img_gt=None, visualize=args.visualize_feature,
    ...
    return f_cor_arr, total_loss, wt_loss
...
def lookahead_train(train_loader, net, optim, curr_epoch, writer, scheduler, max_iter):
    ...
    meta_length = args.length
    alpha = args.alpha

    train_total_loss = AverageMeter()
    time_meter = AverageMeter()

    curr_iter = curr_epoch * (len(train_loader) // meta_length * meta_length)

    train_iter = iter(train_loader)
    for i in range(len(train_loader) // meta_length):
        if curr_iter >= max_iter:
            break
...
def MVRML_train(train_loader, net, optim, curr_epoch, writer, scheduler, max_iter):
    net.train()
...
def log(curr_epoch, curr_iter, f_cor_arr, i, optim, time_meter, train_loader, train_total_loss, writer, wt_loss):
    if args.local_rank == 0:
        if i % 50 == 49:
            if args.visualize_feature:
                visualize_matrix(writer, f_cor_arr, curr_iter, '/Covariance/Feature-')
