...
def load_weights(net, optimizer, scheduler, snapshot_file, restore_optimizer_bool=False):
    ...
    net, optimizer, scheduler, epoch, mean_iu = restore_snapshot(net, optimizer, scheduler, snapshot_file, restore_optimizer_bool)
...
def forgiving_state_restore(net, loaded_dict):
    ...
    for k in net_state_dict:
        if k in loaded_dict:
            if net_state_dict[k].size() == loaded_dict[k].size():
                new_loaded_dict[k] = loaded_dict[k]
            else:
                print("Skipped loading parameter: size mismatch", k)
        else:
            print("Skipped loading parameter", k)
            print(dict(loaded_dict).keys())
