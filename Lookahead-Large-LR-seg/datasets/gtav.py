...
class m:
    @staticmethod
    def imread(p):
        return np.array(imageio.imread(p))[:, :, :3]
...
def make_cv_splits(img_dir_name):
    ...
    train_file = 'split_data/gtav_split_train.txt'
    with open(train_file, 'r') as f:
        lines = f.readlines()
        trn_cities = [line.strip() for line in lines]

    val_file = 'split_data/gtav_split_val.txt'
    with open(val_file, 'r') as f:
        lines = f.readlines()
        val_cities = [line.strip() for line in lines]
...
def make_dataset(mode, maxSkip=0, cv_split=0):
    ...
    #     for c in cv_splits[mode]:
    #         items.append(
    #             (os.path.join(img_path, c, img_postfix),
    #             os.path.join(mask_path, c, mask_postfix))
    #         )
    ...
    if mode == 'train':
        ...
        test_file = 'split_data/gtav_split_test.txt'
        with open(test_file, 'r') as f:
            lines = f.readlines()
        items = [line.strip() for line in lines]
    items = [(os.path.join(root, 'images', item), os.path.join(root, 'labels', item)) for item in items]
